...
function splitCandidateHosts(raw: string): string[] {
	return String(raw || "")
		.split(/[,\n;|]/)
		.map((item) => item.trim())
		.filter(Boolean);
...
export async function syncTime(options?: {
	...
	if (!configuredUrl) {
		throw new Error("请先配置校时地址或 NTP Host");
	}
	...
	const candidateUrls =
		provider === "ntp" ? splitCandidateHosts(configuredUrl) : [configuredUrl];

	let lastError: unknown = new Error("网络校时失败");
	...
	for (const candidateUrl of candidateUrls) {
		try {
			...
			if (candidateUrls.length > 1) {
				...
				logger.warn(`校时节点失败，尝试下一个节点: ${candidateUrl}｜${msg}`);
			}
		}
	}
